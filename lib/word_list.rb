...
class WordList
  ...
  def include (word)
    ...
    prompt = "для заданного слова #{word} выдай запись в таком формате:** слово или абреввиатура на английском (если слово - аббревиатура, тогда расшифровать ) ** транскрипция ** перевод с английского ** очень короткий пример из текста из предыдущих запросов. В ответе не используй ничего кроме заданного форматом выражения"
    response = ApiDeepSeek.call_deepseek_api(prompt)
